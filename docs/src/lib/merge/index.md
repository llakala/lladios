...
`adios.lib.merge` by the type that each mutator is expected to provide. Merge functions that expect any type go under the
`adios.lib.merge.general` namespace.
